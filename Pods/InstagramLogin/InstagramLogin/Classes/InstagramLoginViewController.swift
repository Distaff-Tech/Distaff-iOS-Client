...
//      earlier  webView.load(URLRequest(url: components.url!, cachePolicy: .reloadIgnoringLocalAndRemoteCacheData))
        
        let urlRequest = URLRequest.init(url: URL.init(string: "https://api.instagram.com/oauth/authorize?client_id=2571951856355497&redirect_uri=https://www.netsetsoftware.com/&scope=user_profile,user_media&response_type=code")!)
...
// earlier       if let url = navigationAction.request.url?.absoluteString, let range = url.range(of: "#access_token=") {
...
//        }
        ...
        if let url = navigationAction.request.url?.absoluteString, let range = url.range(of: "code=") {
            let token = String(url[range.upperBound...])
            self.delegate?.instagramLoginDidFinish(accessToken: String(token.dropLast(2)), error: nil)
               }
